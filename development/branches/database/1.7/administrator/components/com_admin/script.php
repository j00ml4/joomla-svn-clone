...
/**
 * @version		$Id: script.php 21884 2011-07-18 19:50:35Z dextercowley $
...
class joomlaInstallerScript
{
	...
	protected function updateManifestCaches()
	{
		...
		foreach ($extensions as $extension) {
			$query->where('type='.$db->quote($extension[0]).' AND element =.$db->quote($extension[1]). AND folder = .$db->quote($extension[2]). AND mclient_id = .$extension[3], OR);
		}
		$db->setQuery($query);
