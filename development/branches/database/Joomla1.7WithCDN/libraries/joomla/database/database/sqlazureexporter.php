...
/**
 * SQLAzure export driver.
 *
 * @package     Joomla.Platform
 * @subpackage  Database
 * @since       11.1
 */
class JDatabaseExporterSQLAzure
{
	/**
	...
	/**
	 * @var    JDatabaseSQLAzure  The database connector to use for exporting structure and/or data.
	 * @since  11.1
	 */
	protected $db = null;
	...
	protected function buildXml()
	{
		...
		$buffer[] = '<sqldump xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">';
	...
	public function check()
	{
		...
		if (!($this->db instanceof JDatabaseSQLAzure)) {
	...
	protected function getColumns($table)
	{
		if (empty($this->cache['columns'][$table])) {
			...
			$this->db->setQuery(
				"SELECT c.name as Field from sys.tables as t inner join 
				sys.columns c on t.OBJECT_ID=c.OBJECT_ID where t.name='".$table."'"
			);
	 ...
	 *
	 * @return  JDatabaseExporterSQLAzure  Method supports chaining.
	 *
	 * @since   11.1
	 ...
	 *
	 * @param   JDatabaseSQLAzure  $db  The database connector.
	 *
	 * @return  JDatabaseExporterSQLAzure  Method supports chaining.
	 *
	 * @since   11.1
