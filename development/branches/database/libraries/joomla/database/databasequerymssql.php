 ...
 * @since		1.6
 */
class JDatabaseQueryMsSQLElementMsSQL
{
	/**
...
class JDatabaseQueryMsSQL
{
	...
	public function select($columns)
	{
		...
		if (is_null($this->_select)) {
			$this->_select = new JDatabaseQueryMsSQLElementMsSQL('SELECT', $columns);
		}
		else {
	...
	{
		$this->_type	= 'delete';
		$this->_delete	= new JDatabaseQueryMsSQLElementMsSQL('DELETE', null);
	...
	{
		$this->_type	= 'insert';
		$this->_insert	= new JDatabaseQueryMsSQLElementMsSQL('INSERT INTO', $tables);
	...
	{
		$this->_type = 'update';
		$this->_update = new JDatabaseQueryMsSQLElementMsSQL('UPDATE', $tables);
	...
	public function from($tables)
	{
		if (is_null($this->_from)) {
			$this->_from = new JDatabaseQueryMsSQLElementMsSQL('FROM', $tables);
		}
		else {
	...
	public function join($type, $conditions)
	{
		...
		$this->_join[] = new JDatabaseQueryMsSQLElementMsSQL(strtoupper($type) . ' JOIN', $conditions);
	...
	public function set($conditions, $glue=',')
	{
		if (is_null($this->_set)) {
			...
			$this->_set = new JDatabaseQueryMsSQLElementMsSQL('SET', $conditions, "\n\t$glue ");
		}
		else {
	...
	public function where($conditions, $glue='AND')
	{
		if (is_null($this->_where)) {
			$glue = strtoupper($glue);
			$this->_where = new JDatabaseQueryMsSQLElementMsSQL('WHERE', $conditions, " $glue ");
		}
		else {
	...
	public function group($columns)
	{
		if (is_null($this->_group)) {
			$this->_group = new JDatabaseQueryMsSQLElementMsSQL('GROUP BY', $columns);
		}
		else {
	...
	public function having($conditions, $glue='AND')
	{
		if (is_null($this->_having)) {
			$glue = strtoupper($glue);
			$this->_having = new JDatabaseQueryMsSQLElementMsSQL('HAVING', $conditions, " $glue ");
		}
		else {
	...
	public function order($columns)
	{
		if (is_null($this->_order)) {
			$this->_order = new JDatabaseQueryMsSQLElementMsSQL('ORDER BY', $columns);
		}
		else {
