<?php
/**
 * @version		$Id: module.php 19243 2010-10-28 08:34:54Z infograf768 $
...
class JInstallerModule extends JAdapterInstance
{
	...
	public function uninstall($id)
	{
		...
		$query = 'DELETE FROM '.$db->nameQuote('#__modules').' WHERE module = '.$db->Quote($row->element) . ' AND client_id = ' . $row->client_id;
		$db->setQuery($query);
	...
	protected function _rollback_menu($arg)
	{
		...
		$query = 'DELETE' .
				' FROM '.$db->nameQuote('#__modules_menu') .
