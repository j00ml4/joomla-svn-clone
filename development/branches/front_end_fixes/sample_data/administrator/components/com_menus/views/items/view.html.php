	...
	public function display($tpl = null)
	{
		...
		foreach ($items as $i => &$item)
		{
			...
			switch ($item->type) {
				...
				default:
					// load language
					$lang->load($item->componentname.'.menu', JPATH_ADMINISTRATOR, null, false, false)
					||	$lang->load($item->componentname.'.menu', JPATH_ADMINISTRATOR.'/components/'.$item->componentname, null, false, false)
					||	$lang->load($item->componentname.'.menu', JPATH_ADMINISTRATOR, $lang->getDefault(), false, false)
					||	$lang->load($item->componentname.'.menu', JPATH_ADMINISTRATOR.'/components/'.$item->componentname, $lang->getDefault(), false, false);
						...
						if (is_file($file) && $xml = simplexml_load_file($file)) {
							...
							if ($view = $xml->xpath('view[1]')) {
								if (!empty($view[0]['title'])) {
									$value .= ' » ' . JText::_((string)$view[0]['title']);
							...
							$value .= ' » ' . JText::_($item->componentname.'_'.$vars['view'].'_view');
						}
					}
					break;	
			}
