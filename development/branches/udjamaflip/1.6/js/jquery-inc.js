...
jQuery.fn.inc = function(url, transform, post) {
 return this.each(function() {
  ...
  var transfer = function(txt) {
   t.html(jQuery.isFunction(transform) ? transform(txt) : txt);
   if (post) {
    post();
   }
  };
  ...
  if (jQuery.browser.msie) {
   ...
   jQuery('<iframe><\/iframe>').hide().attr('id', f).bind('readystatechange', function() {
...
jQuery(function() {
 jQuery('[class~=inc]').each(function() {
  jQuery(this).inc(unescape(this.className.replace(/.*inc:([^ ]+)( .*|$)/, '$1')));
 });
});