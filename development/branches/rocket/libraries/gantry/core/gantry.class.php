...
class Gantry {
	...
	function getParams($prefix=null,$remove_prefix=false) {
        if (null==$prefix){
		    return $this->_templateDetails->params;
        }
        $params=array();
        foreach ($this->_templateDetails->params as $param_name => $param_value){
            $matches = array();
    ...
    function displayBodyTag() {
        ...
        foreach ($this->_bodyclasses as $param) {
        	...
        	if ($param_value != "") {
            	$body_classes[] = $param ."-".$param_value;
            } else {
            	$body_classes[] = $param;
            }
        }
	...
	function _getPresetParams($name,$preset){
		if (array_key_exists('gantry_profile', $_GET)){
			$profile = JProfiler::getInstance('Gantry->_getPresetParams()');
			$profile->mark('Start');
		}
		$return_params = array();
