...
Gantry.PresetsSaver = {
	...
	build: function() {
		var wrapper = new Element('div', {'id': 'presets-namer', 'class': 'gantry-layer-wrapper'}).inject(document.body);
		...
		//var close = new Element('div', {'class': 'gantry-layer-close'}).setHTML('<span>(x)</span>').inject(wrapper);
		...
		hash.each(function(value, key) {
			...
			if (hash.length > 1) var skip = new Element('span', {'class': 'skip'}).set('text', GantryLang['preset_skip']).inject(head);
		...
		Gantry.PresetsSaver.savePreset = new Element('button', {disabled: 'disabled'}).set('text', GantryLang['save']).inject(bottom);
		Gantry.PresetsSaver.cancel = new Element('button').set('text', GantryLang['cancel']).inject(bottom);

		Gantry.PresetsSaver.cancel.addEvent('click', function(e) {
			Gantry.Overlay.removeEvent('show', Gantry.PresetsSaver.bounds.show);
			wrapper.empty().dispose();
			Gantry.Overlay.hide();
		});
	...
	checkInputs: function() {
		...
		if (check || !inputs.length) Gantry.PresetsSaver.savePreset.setProperty('disabled', 'disabled');
		else Gantry.PresetsSaver.savePreset.removeProperty('disabled');

		return check;
	},
	
	save: function() {
		if (!Gantry.PresetsSaver.checkInputs) return;
		var inputs = [];
	...
	handleResponse: function(tree, list, response) {
		...
		if (response == "success") {
			...
			title = new Element('div').set('html', '<h3>'+ GantryLang['success_save'] +'</h3>').inject(success);
		    ...
		    hash.each(function(presetvalue, presetkey) {
				...
				if (i>1) return;
					...
					presetvalue.each(function(param, paramkey) {
						storing[key][inputs[j][1].value][paramkey] = document.id('params'+paramkey).get('value') || '';
					});
				}
				i++;
		    });
