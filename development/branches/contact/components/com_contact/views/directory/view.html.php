 ...
 * @since		1.5
 */
class ContactViewDirectory extends JView
{
	protected $state;
	protected $items;
	protected $category;
	...
	function display($tpl = null)
	{
		...
		if ((!$active) || ((strpos($active->link, 'view=directory') === false) )) {
			if ($layout = $directory->params->get('directory_layout')) {
	...
	protected function _prepareDocument()
	{
		...
		if ($menu && ($menu->query['option'] != 'com_contact' || $menu->query['view'] == 'contact' )) {
			$path = array(array('title' => $this->category->title, 'link' => ''));


			while (($menu->query['option'] != 'com_contact' || $menu->query['view'] == 'contact' ) )
			{
