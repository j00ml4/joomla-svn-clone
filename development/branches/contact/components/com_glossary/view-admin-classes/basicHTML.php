...
class basicHTML extends glossaryHTML {
	var $pageNav = '';
	var $act = '';
	var $limit = 10;
...
INPUT_AREA;
	...
	function formStart ($title, $logo) {
		$interface =& cmsapiInterface::getInstance();
		$componentname = _GLOSSARY_COMPONENT_TITLE;
		?>
		...
		<table cellpadding="4" cellspacing="0" border="0" width="100%">
   		<tr>
			...
			<?php echo $interface->makeImageURI($logo,64,64); ?>
			<span class="sectionname">&nbsp;<?php echo $componentname.' '.$title; ?></span>
			</div>
			</td>
			<td width="25%">
			</td>
...
FORM_END;
	...
	function tooltip ($text) {
		$interface = cmsapiInterface::getInstance();
		return '<a href="javascript:void(0)"  onmouseover="return escape('."'".$text."'".')">'.$interface->makeImageURI('tooltip.png').'</a>';
	}

}

class cmsapiAdminHTML extends basicHTML {
	var $configuration = '';
	var $pageNav = '';
	var $clist = '';
	var $act = '';
	...
	public function __construct ($controller, $limit, $clist, $full_name) {
		...
		$this->configuration = cmsapiConfiguration::getInstance($full_name);
		$this->clist = $clist;
	}
...
INPUT_BOX;
	...
	function editLink ($id, $linkname, $containerid=0) {
		$cname = strtolower(_THIS_COMPONENT_NAME);
		$url = "index2.php?option=com_{$cname}&amp;act={$this->act}&amp;task=edit&amp;cfid={$id}";
