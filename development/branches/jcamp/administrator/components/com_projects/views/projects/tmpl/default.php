<?php
/**
 * @version		$Id: default.php 18740 2010-08-31 19:21:09Z 3dentech $
...
<form action="<?php echo JRoute::_('index.php?option=com_projects&view=projects'); ?>" method="post" name="adminForm" id="adminForm">
	<fieldset id="filter-bar">
		<div class="filter-search fltlft">
			<label class="filter-search-lbl" for="filter_search"><?php echo JText::_('JSEARCH_FILTER_LABEL'); ?></label>
			<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->state->get('filter.search'); ?>" title="<?php echo JText::_('COM_PROJECTS_SEARCH_IN_TITLE'); ?>" />
	...
	<table class="adminlist">
		...
		<?php foreach ($this->items as $i => $item) :
			...
			$item->cat_link	= JRoute::_('index.php?option=com_categories&extension=com_projects&task=edit&type=other&cid[]='. $item->catid);
			$canCreate	= $user->authorise('core.create',		'com_projects.category.'.$item->catid);
			$canEdit	= $user->authorise('core.edit',			'com_projects.category.'.$item->catid);
			$canCheckin	= $user->authorise('core.manage',		'com_checkin') || $item->checked_out==$user->get('id') || $item->checked_out==0;
			$canChange	= $user->authorise('core.edit.state',	'com_projects.category.'.$item->catid) && $canCheckin;
			?>
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td>
					...
					<?php endif; ?>
					<?php if ($canEdit) : ?>
						<a href="<?php echo JRoute::_('index.php?option=com_projects&task=project.edit&id='.(int) $item->id); ?>">
