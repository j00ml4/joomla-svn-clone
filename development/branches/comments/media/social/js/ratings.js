...
{e=new Event(e).stop();var form=$(e.target.getProperty('form'));var context=form.getProperty('id').replace('rate-','');var query='format=json&tmpl=component&format=raw';var action=form.getProperty('action');form.setProperty('action',action.contains('?')?action+'&'+query:action+'?'+query);form.score.value=e.target.getProperty('rel');form.send({onComplete:function(response)
