...
class JHtmlCommentModeration
{
	function action($item)
	{
		...
		if ($item->published == 0) {
			...
			$html[] = '	<label for="moderate_defer_'.$item->id.'">'.JText::_('SOCIAL_DEFER').'</label></li>';
		}
		...
		$html[] = '	<label for="moderate_publish_'.$item->id.'">'.JText::_('SOCIAL_PUBLISH').'</label></li>';
		...
		$html[] = '	<label for="moderate_spam_'.$item->id.'">'.JText::_('SOCIAL_SPAM').'</label></li>';
		...
		$html[] = '	<label for="moderate_delete_'.$item->id.'">'.JText::_('SOCIAL_DELETE').'</label></li>';
