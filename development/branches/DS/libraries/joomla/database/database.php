...
abstract class JDatabase extends JObject
{
	...
	public static function getInstance($options = array())
	{
		...
		if (empty($instances[$signature])) {
			...
			$path	= str_replace('\\','/',dirname(__FILE__)).'/database/'.$driver.'.php';
	...
	public function getConnectors()
	{
		...
		$handlers = JFolder::files(str_replace('\\','/',dirname(__FILE__)).'/database', '.php$');
		...
		foreach($handlers as $handler) {
			...
			if (!class_exists($class)) {
				require_once str_replace('\\','/',dirname(__FILE__)).'/database/'.$name.'.php';
			}
