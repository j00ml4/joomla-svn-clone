<?php
/**
 * @version		$Id: router.php 18109 2010-07-13 11:21:43Z 3dentech $
 * @package		Joomla
...
function MediagalleriesBuildRoute(&$query)
{
	...
	if (isset($view) and ($view == 'gallery' or $view == 'media')) {
		if ($mId != intval($query['id']) || $mView != $view) {
			if($view == 'media' && isset($query['catid']))
			{
				$catid = $query['catid'];
			...
			$menuCatid = $mId;
			$categories = JCategories::getInstance('mediaggalleries');
...
function MediagalleriesParseRoute($segments)
{
	...
	$category = JCategories::getInstance('Mediagalleries')->get($id);
	...
	foreach($segments as $segment)
	{
		...
		if ($found == 0)
		{
			...
			{
				$db = JFactory::getDBO();
				$query = 'SELECT id FROM #__medias WHERE catid = '.$vars['id'].' AND alias = '.$db->Quote(str_replace(':', '-',$segment));
				$db->setQuery($query);
				$id = $db->loadResult();
			} else {
				$id = $segment;
