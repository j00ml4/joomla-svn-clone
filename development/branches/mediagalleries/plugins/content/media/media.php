...
class plgContentMedia extends JPlugin
{
	 ...
	 */
	public function addMedia( $media, $width='', $height ='', $autostart=1 )
	{
		...
		$params['autostart']=(boolean)$autostart;
		$params['width']=$width;
		...
		if(ctype_alnum($type)){ // If the type is alphanumeric...
			...
			if(!file_exists(dirname(__FILE__) . DS ."types".DS.$type.".php")){
				//This means that we dont know how to handle this extension..
				//USe default..
				$temp = self::getMediaObject("default");				
				return $temp->getMedia($media, $params);
			}
			else {
				//This means that we KNOW how to handle this extension..
				$temp = self::getMediaObject($type);
				return $temp->getMedia($media, $params);
			}
			...
			if(!file_exists(dirname(__FILE__) . DS ."types".DS.$host.".php")){
				...
				//This means that we KNOW how to handle this host..
				$temp = self::getMediaObject($host);
				return  $temp->getMedia($media, $params);	
			}
		}
	}
